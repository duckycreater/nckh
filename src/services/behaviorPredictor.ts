...
const SNN_BASE =
  (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_SNN_URL) ||
  "http://localhost:8002";
...
class BehaviorPredictor {
  ...
  predictLocal(features: BehaviorFeatures): DropoutPrediction {
    let risk = 0;
    risk += Math.min(1, features.last_active_hours / 72) * 0.3;
    risk += Math.min(1, features.missed_days_7d / 5) * 0.3;
    risk += (1 - features.novelty_score) * 0.2;
    risk += (1 - Math.min(1, features.daily_sessions / 3)) * 0.2;
    ...
    return {
      ...
      recommended_intervention:
        interventions[Math.floor(risk * interventions.length) % interventions.length],
