...
import { getDailyChallengeIds } from "../lib/dailyChallenges";
...
const CHALLENGE_POOL = [
  {
    id: 1,
    titleKey: "dailyChallenges.challenges.personalBottle",
  ...
  {
    id: 2,
    titleKey: "dailyChallenges.challenges.turnOffLight",
  ...
  {
    id: 3,
    titleKey: "dailyChallenges.challenges.aiExpert",
  ...
  {
    id: 4,
    titleKey: "dailyChallenges.challenges.noPlastic",
  ...
  {
    id: 5,
    titleKey: "dailyChallenges.challenges.recycleProper",
  ...
  {
    id: 6,
    titleKey: "dailyChallenges.challenges.walkOrBike",
  ...
  {
    id: 7,
    titleKey: "dailyChallenges.challenges.saveEnergy",
...
type DailyChallenge = (typeof CHALLENGE_POOL)[number];
  ...
  if (stored) {
    try {
      ...
      if (parsed.date === today && Array.isArray(parsed.challenges)) {
        return parsed.challenges as DailyChallenge[];
      }
    } catch {
      /* ignore */
    }
  }
  const result = getDailyChallengeIds(today)
    .map((id) => CHALLENGE_POOL.find((challenge) => challenge.id === id))
    .filter((challenge): challenge is DailyChallenge => challenge !== undefined);
  localStorage.setItem("dailyChallengeSet", JSON.stringify({ date: today, challenges: result }));
  return result;
}

export function DailyChallenges({ userId, progress, onRefresh }: Props) {
  const { t } = useTranslation();
  const [completed, setCompleted] = useState<number[]>([]);
  const [completedAnim, setCompletedAnim] = useState<number | null>(null);
  const [chainCount, setChainCount] = useState(() => {
    try {
      return parseInt(localStorage.getItem("bmo:challengeChain") || "0", 10);
    } catch {
      return 0;
    }
  });
  ...
  const handleComplete = async (id: number) => {
    ...
    try {
      ...
      const nextCompleted = Array.isArray(result.progress?.challengesCompleted)
        ? result.progress.challengesCompleted
        : [...completed, id];
      const newChain = nextCompleted.length;
      setChainCount(newChain);
      localStorage.setItem("bmo:challengeChain", String(newChain));
      setCompleted(nextCompleted);
      setCompletedAnim(id);
      onRefresh?.(result.progress);
      if (Number(result.earnedPoints) >= CHAIN_BONUS) {
        showToast(t("dailyChallenges.chainComplete"), Number(result.earnedPoints));
      }
      setTimeout(() => setCompletedAnim(null), 400);
    } catch {
      // Keep the challenge unclaimed so the user can retry after reconnecting.
    }
  };
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h4
                  className={`text-sm font-semibold truncate ${isDone ? "text-gray-400 line-through" : "text-gray-700"}`}
                >
                  {t(c.titleKey)}
                </h4>
