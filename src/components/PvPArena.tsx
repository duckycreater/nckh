...
function buildBattleCard(id: number, level = 5): BattleCard {
  ...
  const def = Math.floor((base.def || 5) * (1 + (level - 1) * 0.1));
    ...
    moves: [
      {
        ...
        cooldown: 0,
        currentCooldown: 0,
        power: Math.floor(atk * 0.9),
      },
      {
        id: "skill",
        name: "Kỹ Năng",
        desc: "Đòn mạnh +15% DEF",
        icon: "💥",
        type: "skill",
        energyCost: 25,
        cooldown: 2,
        currentCooldown: 0,
        power: Math.floor(atk * 1.4),
      },
      {
        id: "defend",
        name: "Phòng Thủ",
        ...
        currentCooldown: 0,
        power: 0,
        effect: { type: "shield", value: Math.floor(def * 2 + level * 12), duration: 2 },
      },
      {
        id: "ultimate",
        name: "Chiêu Cuối",
...
function buildOpponentCard(
  id: number,
  tier: RivalTier = "bronze",
  tierConfig?: RivalTierConfig,
): BattleCard {
  ...
  if (!base) {
    return {
      ...
      elementId: "hazard",
      rarityId: "epic",
      atk: Math.floor(25 * tc.atkMult),
      hp: Math.floor(150 * tc.hpMult),
      maxHp: Math.floor(150 * tc.hpMult),
      def: Math.floor(5 * tc.defMult),
      spd: Math.floor(5 * tc.spdMult),
      crt: Math.min(40, 5 + tc.crtBonus),
      int: 5,
      level: 5,
      isAlive: true,
      moves: [],
  ...
  const defBase = Math.floor((base.def || 5) * (1 + (level - 1) * 0.1));
  ...
  if (hasStun)
    moves.push({
      id: "stun_hit",
      name: "Khiến Choáng",
      desc: "Có thể làm choáng",
      icon: "⚡",
      type: "skill",
      energyCost: 30,
      cooldown: 3,
      currentCooldown: 0,
      power: Math.floor(atk * 1.0),
  ...
  if (hasDefDown)
    moves.push({
      ...
      cooldown: 2,
      currentCooldown: 0,
      power: Math.floor(atk * 0.7),
      effect: { type: "def_down", value: 15 },
    });
  if (hasHeal)
    moves.push({
      id: "heal_up",
      ...
      currentCooldown: 0,
      power: 0,
      effect: { type: "heal", value: Math.floor(hp * 0.25) },
    });
  if (hasUltimate)
    moves.push({
      id: "ultimate",
...
function pickSmartMove(
  ...
  const targetIdx = playerTeam.findIndex(
    (c) => c.isAlive && c.hp === Math.min(...alivePlayers.map((p) => p.hp)),
  );
...
function calcDamage(
  ...
  // Evasion
  if (Math.random() * 100 < (defender.sp ?? 0) * 0.5) {
    notes.push("Trượt!");
    return { dmg: 0, isCrit: false, notes };
  }
...
function calcDmg(
  ...
  if (Math.random() * 100 < defender.spd * 0.5) {
    notes.push("Trượt!");
    return { dmg: 0, isCrit: false, notes };
  }
...
export function PvPArena({ onClose, onBattle, currentUserNick, userCards }: Props) {
  ...
  const handleStartPvP = () => {
    ...
    const oTeam =
      oppPool.length >= 3
        ...
        : [
            buildOpponentCard(cards[0] || 1, rival.tier, tierConfig),
            buildOpponentCard(cards[1] || 2, rival.tier, tierConfig),
            buildOpponentCard(cards[2] || 3, rival.tier, tierConfig),
          ];
  ...
  const handlePvPEnd = useCallback(
    (playerWon: boolean, finalP?: BattleCard[], finalO?: BattleCard[], round?: number) => {
    ...
    [rivalProfile, rankStreak, playerRank, turnCount, playerTeam, opponentTeam, onBattle],
  );
  ...
  const executeOpponentMove = useCallback(
    (
      ...
      if (newO[oIdx].poisonStacks > 0) {
        ...
        newO[oIdx].hp -= pd;
        newLog.push(`☠️ ${newO[oIdx].name} nhận ${pd} độc!`);
      }
      if (newO[oIdx].burnStacks > 0) {
        const bd = Math.floor(newO[oIdx].maxHp * 0.03 * newO[oIdx].burnStacks);
        newO[oIdx].hp -= bd;
        newLog.push(`🔥 ${newO[oIdx].name} nhận ${bd} cháy!`);
      }

      if (newO[oIdx].hp <= 0) {
        newO[oIdx].hp = 0;
        newO[oIdx].isAlive = false;
      ...
      if (
        !newO[oIdx].isAlive &&
        newO[oIdx].specialAbility === "second_wind" &&
        !newO[oIdx].secondWindUsed
      ) {
        ...
        !newO[oIdx].isAlive &&
        newO[oIdx].specialAbility === "last_stand" &&
        !newO[oIdx].lastStandUsed
      ) {
        newO[oIdx].isAlive = true;
        ...
        !newO[oIdx].isAlive &&
        newO[oIdx].specialAbility === "iron_will" &&
        !newO[oIdx].ironWillUsed
      ) {
        newO[oIdx].isAlive = true;
        newO[oIdx].hp = Math.floor(newO[oIdx].maxHp * 0.3);
      ...
      if (realOIdx !== oIdx) {
        oIdx = realOIdx;
        setCurrentOpponentIdx(realOIdx);
      }
      ...
      if (!move) {
        ...
        if (move.type === "dodge") {
          newO[oIdx].dodgeActive = true;
          newO[oIdx].dodgeCooldown = 3;
          ...
          newP[pIdx].defDownStacks =
            (newP[pIdx].defDownStacks || 0) + Math.floor(move.effect.value / 5);
          ...
          const { dmg, isCrit, notes } = calcDmg(
            { ...newO[oIdx], atk: effAtk },
            newP[pIdx],
            move.power,
            false,
          );
    ...
    [rivalEnraged, turnCount, handlePvPEnd],
  );
  ...
  const executePlayerMove = useCallback(() => {
    ...
    // Apply move
    if (move.type === "dodge") {
      newP[pIdx].dodgeActive = true;
      newP[pIdx].dodgeCooldown = 3;
      ...
      newO[oIdx].defDownStacks =
        (newO[oIdx].defDownStacks || 0) + Math.floor(move.effect.value / 5);
        ...
        <div className={`px-4 py-3 flex items-center justify-between bg-gradient-to-r ${stageBg}`}>
          <div className="flex items-center gap-2">
            {stage !== "idle" && stage !== "team_select" && (
              <button
                onClick={() => (stage === "battling" ? null : setStage("idle"))}
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-white/60">
                Đấu trường 1v1 · Thẻ bài đối kháng
              </p>
            </div>
          </div>
          {stage === "battling" && (
            ...
            >
              <div className="text-center mb-6">
                ...
                <div className="mb-2 flex items-center justify-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-black ${playerRank === "diamond" ? "bg-cyan-500/20 text-cyan-300" : playerRank === "gold" ? "bg-amber-500/20 text-amber-300" : playerRank === "silver" ? "bg-slate-400/20 text-slate-300" : "bg-orange-700/30 text-orange-400"}`}
                  >
                  ...
                  return (
                    ...
                    >
                      <span className="text-2xl">
                        {getElementIcon(card?.elementId || "hazard")}
                      </span>
            ...
            >
              ...
              <div className="flex items-center justify-between rounded-xl border border-slate-700 bg-slate-800 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-1.5 py-0.5 text-[9px] font-black ${playerRank === "diamond" ? "bg-cyan-500/20 text-cyan-300" : playerRank === "gold" ? "bg-amber-500/20 text-amber-300" : playerRank === "silver" ? "bg-slate-400/20 text-slate-300" : "bg-orange-700/30 text-orange-400"}`}
                  >
                    {RIVAL_TIERS.find((t) => t.tier === playerRank)?.label || "Bronze"}
                  </span>
                  {rankStreak > 0 && (
                  ...
                  <span className="text-xs text-slate-500">
                    · {playerTeam.filter((c) => c.isAlive).length}/3
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-500">
                    {opponentTeam.filter((c) => c.isAlive).length}/3 ·
                  </span>
                  <span
                    className={`rounded-full px-1.5 py-0.5 text-[9px] font-black ${rivalProfile.tier === "diamond" ? "bg-cyan-500/20 text-cyan-300" : rivalProfile.tier === "gold" ? "bg-amber-500/20 text-amber-300" : rivalProfile.tier === "silver" ? "bg-slate-400/20 text-slate-300" : "bg-orange-700/30 text-orange-400"}`}
                  >
                    {RIVAL_TIERS.find((t) => t.tier === rivalProfile.tier)?.label || "Bronze"}
                  </span>
                  <Target
                    size={16}
                    className={rivalEnraged ? "text-amber-400 animate-pulse" : "text-red-400"}
                  />
                  <span
                    className={`text-sm font-bold ${rivalEnraged ? "text-amber-400" : "text-red-400"}`}
                  >
                    {rivalProfile.name}
                    {rivalEnraged && " ⚡"}
                  </span>
              ...
              <div className="rounded-xl border border-red-800 bg-red-950/40 p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] font-bold text-red-500">
                        {RIVAL_TIERS.find((t) => t.tier === rivalProfile.tier)?.label || "Bronze"} ·
                        LV{currentOppCard.level}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-black text-red-300">
                      {Math.max(0, currentOppCard.hp)}/{currentOppCard.maxHp}
                    </p>
                ...
                <div className="h-3 w-full overflow-hidden rounded-full bg-red-900/60">
                  <motion.div
                    animate={{
                      width: `${Math.max(0, (currentOppCard.hp / currentOppCard.maxHp) * 100)}%`,
                    }}
                    className="h-full rounded-full bg-gradient-to-r from-red-600 to-orange-500"
                  />
                </div>
                <div className="mt-1 flex gap-2">
                  {currentOppCard.poisonStacks > 0 && (
                    <span className="text-[10px] text-green-400">
                      ☠️x{currentOppCard.poisonStacks}
                    </span>
                  )}
                  {currentOppCard.burnStacks > 0 && (
                    <span className="text-[10px] text-orange-400">
                      🔥x{currentOppCard.burnStacks}
                    </span>
                  )}
                  {currentOppCard.shieldActive && (
                    <span className="text-[10px] text-blue-400">
                      🛡️{currentOppCard.shieldValue}
                    </span>
                  )}
                  {currentOppCard.stunned > 0 && (
                    <span className="text-[10px] text-yellow-400">⚡stun</span>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-emerald-800 bg-emerald-950/40 p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] font-bold text-emerald-500">
                        LV{currentPlayerCard.level}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-black text-emerald-300">
                      {Math.max(0, currentPlayerCard.hp)}/{currentPlayerCard.maxHp}
                    </p>
                ...
                <div className="h-3 w-full overflow-hidden rounded-full bg-emerald-900/60">
                  <motion.div
                    animate={{
                      width: `${Math.max(0, (currentPlayerCard.hp / currentPlayerCard.maxHp) * 100)}%`,
                    }}
                ...
                <div className="mt-1 flex gap-2 text-[10px] text-slate-400">
                  ...
                  {currentPlayerCard.poisonStacks > 0 && (
                    <span className="text-green-400">☠️x{currentPlayerCard.poisonStacks}</span>
                  )}
                  {currentPlayerCard.burnStacks > 0 && (
                    <span className="text-orange-400">🔥x{currentPlayerCard.burnStacks}</span>
                  )}
                  {currentPlayerCard.shieldActive && (
                    <span className="text-blue-400">🛡️{currentPlayerCard.shieldValue}</span>
                  )}
              ...
              <div className="grid grid-cols-2 gap-2">
                {currentPlayerCard.moves.map((move) => {
                  ...
                  const disabled =
                    !canAfford || onCd || stunned || turn !== "idle" || !isPlayerTurn;
                  return (
                    <button
                      key={move.id}
            ...
            >
              {battleResult.won ? (
                <>
                  ...
                  <div className="mb-2 flex items-center justify-center gap-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-black ${rivalProfile.tier === "diamond" ? "bg-cyan-500/20 text-cyan-300" : rivalProfile.tier === "gold" ? "bg-amber-500/20 text-amber-300" : "bg-slate-500/20 text-slate-300"}`}
                    >
                      {RIVAL_TIERS.find((t) => t.tier === rivalProfile.tier)?.label || "Bronze"}
                    </span>
                  ...
                  {rankStreak >= 0 && (
                    <p className="mb-4 text-xs text-slate-500">
                      Chuỗi thắng:{" "}
                      <span className="font-black text-amber-400">🔥 {rankStreak}/3</span>
                  ...
                  >
                    <Zap size={24} className="fill-current" />+{battleResult.reward} EXP
                  </motion.div>
                </>
              ) : (
                <>
                  ...
                  <div className="mb-2 flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm text-slate-500">
                      {rivalProfile.name} đã chiến thắng
                    </span>
                  </div>
              ...
              <div className="mb-6 grid grid-cols-2 gap-2 text-center">
                {[
                  [
                    "Sát thương",
                    playerTeam.reduce((s, c) => s + c.totalDamage, 0),
                    "text-amber-400",
                  ],
