      ...
      effect: {
        type: isDef
          ...
          : isOff
            ...
            : ab.effect.type === "heal"
              ? "heal"
              : ab.effect.type === "poison"
                ? "poison"
                : ab.effect.type === "shield"
                  ? "shield"
                  : ab.effect.type === "regen"
                    ? "regen"
                    : "damage",
...
function calcBattleDamage(
  ...
  const baseDmg = Math.floor(power * (0.85 + Math.random() * 0.3) * intBonus * comboMult);
...
function ScreenShake({ active, children }: { active: boolean; children: React.ReactNode }) {
  return (
    <motion.div
      animate={active ? { x: [-10, 10, -8, 8, -4, 4, 0] } : {}}
...
function HpBar({
  ...
  return (
    <div className="relative w-full">
      <div
        className="w-full overflow-hidden rounded-full bg-black/60 ring-1 ring-white/10"
        style={{ height }}
      >
        <motion.div
...
function EnergyBar({ current, max }: { current: number; max: number }) {
  ...
  return (
    <div className="relative w-full">
      <div
        className="w-full overflow-hidden rounded-full bg-black/60 ring-1 ring-cyan-500/20"
        style={{ height: 6 }}
      >
        <motion.div
          className="rounded-full"
...
function UltimateBar({ charge }: { charge: number }) {
  ...
  return (
    <div className="relative w-full">
      <div
        className="w-full overflow-hidden rounded-full bg-black/60 ring-1 ring-amber-500/20"
        style={{ height: 6 }}
      >
        <motion.div
          className="rounded-full"
          style={{
            ...
            boxShadow: ready ? "0 0 10px #f59e0b" : "none",
          }}
...
function StatusIcon({
  ...
  const colors = STATUS_COLORS[effect] || {
    bg: "bg-gray-900/90",
    text: "text-gray-300",
  ...
  return (
    <div
      className={`flex items-center gap-0.5 rounded-full border px-1.5 py-0.5 text-[9px] font-black ${colors.bg} ${colors.text} ${colors.border}`}
    >
...
function StatusIcons({ card, compact = false }: { card: BattleCard; compact?: boolean }) {
  ...
  if (card.poisonStacks > 0)
    icons.push(<StatusIcon key="poison" effect="poison" stacks={card.poisonStacks} />);
  if (card.burnStacks > 0)
    icons.push(<StatusIcon key="burn" effect="burn" stacks={card.burnStacks} />);
  if (card.shieldActive)
    icons.push(<StatusIcon key="shield" effect="shield" value={card.shieldValue} />);
  if (card.stunned > 0) icons.push(<StatusIcon key="stun" effect="stun" stacks={card.stunned} />);
  if (card.dodgeCooldown > 0)
    icons.push(<StatusIcon key="dodge" effect="dodge" stacks={card.dodgeCooldown} />);
  if (card.regenStacks > 0)
    icons.push(<StatusIcon key="regen" effect="regen" stacks={card.regenStacks} />);
  if (card.silenced > 0)
    icons.push(<StatusIcon key="silenced" effect="silenced" stacks={card.silenced} />);
  if (icons.length === 0) return null;
  return (
    <div
      className={`flex items-center gap-1 flex-wrap ${compact ? "justify-end" : "justify-center"}`}
    >
      {icons}
    </div>
  );
    ...
    <div className="w-full rounded-2xl border border-red-800/40 bg-gradient-to-r from-red-950/80 via-slate-900/90 to-red-950/80 p-3 shadow-[0_0_30px_rgba(239,68,68,0.2)]">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs sm:text-sm font-black text-white leading-none">{boss.name}</p>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="rounded-full bg-red-900/80 border border-red-500/40 px-1.5 py-0.5 text-[8px] sm:text-[9px] font-black text-red-300">
                Lv.{boss.level}
              </span>
              {el && (
                <span className="text-[8px] sm:text-[9px] font-bold" style={{ color: el.accent }}>
                  {el.name}
                </span>
              )}
            </div>
        ...
        <div className="text-right">
          ...
          <p className="text-[8px] sm:text-[9px] text-slate-500">/ {boss.maxHp} HP</p>
          <p className="text-[9px] sm:text-[10px] font-black" style={{ color: barColor }}>
            {pct.toFixed(0)}%
          </p>
        </div>
      </div>
      ...
      <div className="relative mb-1.5">
        <div
          className="w-full overflow-hidden rounded-full bg-black/70 ring-1 ring-white/10"
          style={{ height: 16 }}
        >
          <motion.div
            className="rounded-full"
      ...
      {boss.shieldActive && (
        <div className="flex items-center gap-2 mb-1.5">
          <div className="flex items-center gap-1 rounded-full bg-blue-900/80 border border-blue-500/40 px-2 py-0.5">
            ...
            <span className="text-[9px] sm:text-[10px] font-black text-blue-300">
              Khiên: {boss.shieldValue}
            </span>
            <span className="text-[8px] sm:text-[9px] text-blue-400/70">({boss.shieldTurns}t)</span>
          </div>
        </div>
      )}
...
function MoveButton({
  ...
  return (
    <motion.button
      ...
      disabled={!isReady}
      className={`
        relative flex flex-col items-center justify-center rounded-2xl border-2 p-2 sm:p-2.5
        transition-all duration-150 select-none overflow-hidden min-w-[64px] sm:min-w-[76px] min-h-[60px] sm:min-h-[72px]
    ...
    >
      ...
      {isReady && (
        <div
          ...
          style={{
            background: `radial-gradient(circle at center, ${glowColor}60, transparent 70%)`,
          }}
        />
      )}
      ...
      <div className="absolute top-1 left-1 right-1 text-center">
        <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-wider text-white/60">
          {typeLabel}
        </span>
      </div>

      {/* Icon */}
      <span className="text-xl sm:text-2xl leading-none mt-2">{move.icon}</span>

      {/* Name */}
      <span className="text-[9px] sm:text-[10px] font-black text-white/90 leading-tight text-center mt-0.5 px-0.5">
        {move.name}
      </span>
      ...
      <div className="flex items-center gap-1 mt-0.5">
        {move.energyCost > 0 && (
          <span
            className={`text-[9px] sm:text-[10px] font-black ${noEnergy ? "text-red-400" : "text-cyan-300"}`}
          >
            ⚡{move.energyCost}
          </span>
        )}
    ...
    >
      ...
      <div className="relative rounded-2xl bg-gradient-to-b from-amber-700 via-orange-700 to-red-900 px-4 py-2 shadow-[0_0_30px_rgba(245,158,11,0.6)] border border-amber-400/50">
        ...
        <div className="relative">
          <p className="text-center text-lg sm:text-xl font-black text-amber-200 leading-none">
            🔥 {combo}x
          </p>
          <p className="text-[8px] sm:text-[9px] font-black text-amber-600 uppercase tracking-wider text-center">
            COMBO
          </p>
          <p className="text-[9px] sm:text-[10px] font-black text-red-300 text-center mt-0.5">
            MAX {maxDmg} DMG
          </p>
          {combo >= 2 && (
            <p className="text-[8px] sm:text-[9px] font-black text-yellow-300 text-center bg-yellow-900/50 rounded px-1 mt-0.5">
              ⚡ +{bonusMult}x DMG
            </p>
          )}
        </div>
...
function TypeBadge({ elementId }: { elementId: string }) {
  ...
  return (
    <div
      className="flex items-center gap-1 rounded-full px-2 py-0.5 text-white text-[9px] font-black"
...
function BattleAvatar({
  ...
  const glowColor =
    card.elementId === "hazard"
      ? "#ef4444"
      : card.elementId === "organic"
        ? "#22c55e"
        : card.elementId === "metal"
          ? "#64748b"
          : card.elementId === "glass"
            ? "#14b8a6"
            : card.elementId === "paper"
              ? "#f59e0b"
              : "#06b6d4";
  ...
  function getAvatarAnim(): any {
    ...
    if (isDodging)
      return { x: team === "player" ? 80 : -80, opacity: 0.2, transition: { duration: 0.2 } };
    if (attackAnim)
      return { x: team === "player" ? 100 : -100, transition: { duration: 0.3, ease: "easeOut" } };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Card art body */}
        <div
          className="relative flex items-center justify-center overflow-hidden rounded-xl border-2 backdrop-blur-sm w-16 h-24 sm:w-20 sm:h-28 md:w-[110px] md:h-[148px]"
          style={{
          ...
          {dead && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60 rounded-xl">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[9px] sm:text-[10px] font-black text-red-400">KO</span>
              </div>
            </div>
          )}
            ...
            <>
              ...
              {card.poisonStacks > 0 && (
                <div className="absolute inset-0 z-10 pointer-events-none rounded-xl overflow-hidden">
                  {[...Array(card.poisonStacks)].map((_, i) => (
                    <motion.div
                      key={`poison-${i}`}
                ...
                <div className="absolute inset-0 z-10 pointer-events-none rounded-xl overflow-hidden">
                  {[...Array(card.burnStacks)].map((_, i) => (
                    <motion.div
                      ...
                      animate={{ y: [0, -6, 0], opacity: [0.5, 0.9, 0.5], scale: [0.9, 1.1, 0.9] }}
              ...
              {card.shieldActive && (
                <div className="absolute inset-0 z-10 pointer-events-none rounded-xl">
                  <motion.div
                    ...
                    style={{ borderColor: "#60a5fa", boxShadow: "0 0 12px #60a5fa60" }}
                  />
                </div>
              )}
              {/* Stun — yellow lightning */}
              {card.stunned > 0 && (
              ...
              {card.regenStacks > 0 && (
                <div className="absolute inset-0 z-10 pointer-events-none rounded-xl">
                  <motion.div
                    ...
                    style={{ background: "radial-gradient(circle, #22c55e30 0%, transparent 70%)" }}
                  />
                </div>
              )}
            </>
      ...
      {/* HP Bar */}
      <div className="w-16 sm:w-[108px]">
        <HpBar current={card.hp} max={card.maxHp} accent={glowColor} height={9} />
        <div className="flex justify-between">
          <span
            className={`text-[9px] sm:text-[10px] font-black ${team === "player" ? "text-emerald-400" : "text-red-400"}`}
          >
            {Math.max(0, card.hp)}/{card.maxHp}
          </span>
          <span className="text-[8px] sm:text-[9px] font-black text-slate-500">LV{card.level}</span>
        </div>
      </div>
...
function DamageNumber({ num }: { num: DmgNum }) {
  ...
  return (
    <motion.div
      ...
      style={{
        color,
        textShadow: `0 0 20px ${color}, 0 0 8px rgba(0,0,0,1), 0 3px 10px rgba(0,0,0,1)`,
      }}
    >
      <div
        className="text-3xl sm:text-5xl leading-none font-black drop-shadow-lg"
        style={{
          WebkitTextStroke: num.isCrit ? "1px rgba(0,0,0,0.6)" : "0px",
          textShadow: `0 0 20px ${color}, 0 0 10px rgba(0,0,0,1), 0 3px 10px rgba(0,0,0,1)`,
        }}
      >
        {valueStr}
      </div>
      {label && (
        <div
          className="text-[10px] sm:text-[12px] font-black uppercase tracking-wider mt-0.5"
...
function EffectivenessBanner({
  ...
  return (
    <motion.div
      ...
      style={{
        background: `${info.color}ee`,
        boxShadow: `0 0 28px ${info.color}80`,
        top: "40%",
        [side === "player" ? "left" : "right"]: "2%",
      }}
    >
      {info.label}
    </motion.div>
...
function TurnAnnouncement({ text, color }: { text: string; color: string }) {
  return (
    <motion.div
      ...
      style={{
        background: `${color}ee`,
        boxShadow: `0 0 40px ${color}80, 0 8px 32px rgba(0,0,0,0.5)`,
      }}
    >
      {text}
    </motion.div>
...
function VictoryConfetti() {
  const particles = Array.from({ length: 100 }, (_, i) => ({
    ...
    color: ["#f59e0b", "#22c55e", "#3b82f6", "#a855f7", "#ef4444", "#f97316", "#06b6d4", "#ec4899"][
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div
            className="w-32 h-44 sm:w-40 sm:h-56 md:w-[180px] md:h-[240px] rounded-2xl overflow-hidden border-4"
            style={{
              borderColor: glowColor + "80",
              background: `linear-gradient(145deg, ${glowColor}18, ${glowColor}08)`,
            }}
          >
...
interface Props {
  ...
  onClose: () => void;
  onWin: (claim: GameplayRewardClaim) => void;
}
...
export function CardBattle({ deckCardIds, cardLevels, onClose, onWin }: Props) {
  ...
  const [effectBanner, setEffectBanner] = useState<{
    info: { label: string; color: string };
    side: "player" | "boss";
  } | null>(null);
  ...
  const [bossTelegraph, setBossTelegraph] = useState(false);
  const [bossNextMove, setBossNextMove] = useState<string | null>(null);
  ...
  useEffect(() => {
    pRef.current = playerTeam;
  }, [playerTeam]);
  useEffect(() => {
    bRef.current = bossTeam;
  }, [bossTeam]);
  useEffect(() => {
    energyRef.current = playerEnergy;
  }, [playerEnergy]);
  ...
  const addLog = useCallback((text: string, type: BattleLogEntry["type"] = "info") => {
    ...
    setLog((prev) => [
      ...prev.slice(-14),
      { id: ++logIdRef.current, text, type, color: colors[type] ?? "#94a3b8" },
    ]);
  }, []);
  ...
  const showBanner = useCallback(
    (info: { label: string; color: string }, side: "player" | "boss") => {
  ...
  const applyDamage = useCallback(
    (team: "player" | "boss", idx: number, dmg: number): { newHp: number; died: boolean } => {
      ...
      const newHp = Math.max(
        0,
        (team === "player" ? pRef.current[idx].hp : bRef.current[idx].hp) - actualDmg,
      );
      const died = newHp === 0;
      if (team === "player") {
        setPlayerTeam((prev) =>
          prev.map((c, i) => (i === idx ? { ...c, hp: newHp, isAlive: newHp > 0 } : c)),
        );
      } else {
        setBossTeam((prev) =>
          prev.map((c, i) => (i === idx ? { ...c, hp: newHp, isAlive: newHp > 0 } : c)),
  ...
  const tickStatus = useCallback(
    (team: "player" | "boss", idx: number, card: BattleCard) => {
              ...
              prev.map((c, i) =>
                i === idx
                  ? {
                      ...
                      regenStacks: Math.max(0, c.regenStacks - 1),
                    }
                  : c,
              ),
            );
      ...
      if (Object.keys(updates).length > 0) {
        if (team === "player") {
          setPlayerTeam((prev) =>
            prev.map((c, i) => (i === idx ? ({ ...c, ...updates } as BattleCard) : c)),
          );
        } else {
          setBossTeam((prev) =>
            prev.map((c, i) => (i === idx ? ({ ...c, ...updates } as BattleCard) : c)),
  ...
  const runBossAI = useCallback(() => {
    ...
    function executeBossAction() {
      ...
      if (bossHpPct <= 0.25 && !bossEnraged) {
        setBossEnraged(true);
        announce("🔥 NỔI GIẬN!", "#ef4444");
        addLog(`⚠️ ${boss.name} nổi giận! ATK +30%!`, "status");
        setBossTeam((prev) =>
          prev.map((c, i) => (i === bossIdx ? { ...c, atk: Math.floor(c.atk * 1.3) } : c)),
        );
      }

      // ── TELEGRAPH: if previous turn telegraphed a heavy attack, execute it now ──
      if (bossTelegraph && bossNextMove === "boss-heavy") {
        setBossTelegraph(false);
        setBossNextMove(null);
        executeHeavyAttack();
        return;
      }

      // ── Helper: execute heavy attack ──
      function executeHeavyAttack() {
        const heavyMove = { ...bossMoves[3], power: Math.floor(boss.atk * 1.6) };
        ...
        setTimeout(() => {
          ...
          const { dmg, isCrit, notes } = calcBattleDamage(boss, targetPlayer, heavyMove.power);
          ...
          if (died) {
            ...
            setTimeout(() => {
              ...
              const newIdx =
                nextAlive !== -1 ? nextAlive : pRef.current.findIndex((c) => c.isAlive);
      ...
      if (boss.stunned > 0) {
        ...
        setTimeout(() => {
          setBossTeam((prev) =>
            prev.map((c, i) => (i === bossIdx ? { ...c, stunned: Math.max(0, c.stunned - 1) } : c)),
          );
          tickStatus("boss", bossIdx, boss);
          setTimeout(() => {
            setPlayerTeam((prev) =>
            ...
            setBossTeam((prev) =>
              prev.map((c) => ({ ...c, ultimateCharge: Math.min(100, c.ultimateCharge + 33) })),
            );
            setPlayerEnergy(100);
            if (!checkEnd()) {
              setPhase("idle");
      ...
      const targetPlayer = alivePlayers.reduce(
        (lowest, c) => (c.hp < lowest.hp ? c : lowest),
        alivePlayers[0],
      );
      ...
      const bossMoves: BattleMove[] = [
        {
          ...
          currentCooldown: 0,
          power: boss.atk,
          effect: { type: "damage", value: boss.atk },
        },
        {
          id: "boss-elemental",
          name:
            boss.elementId === "hazard"
              ? "Chất Độc"
              : boss.elementId === "organic"
                ? "Hơi Nóng"
                : "Bão Nguyên Tố",
          desc: "Đòn nguyên tố",
          icon: "🌪️",
          ...
          effect: { type: boss.elementId === "hazard" ? "poison" : "burn", value: 2, duration: 2 },
        },
        {
          id: "boss-dodge",
          name: "Phòng Thủ",
          ...
          currentCooldown: 0,
          power: 0,
          effect: { type: "shield", value: 25, duration: 2 },
        },
        {
          id: "boss-heavy",
          name: "Đòn Nặng",
          ...
          cooldown: 2,
          currentCooldown: 0,
          power: Math.floor(boss.atk * 1.6),
          effect: { type: "damage", value: Math.floor(boss.atk * 1.6) },
        },
      ];
      ...
      // ── TELEGRAPH: warn before heavy attack ──
      if (chosenMove.id === "boss-heavy" && !bossTelegraph) {
        setBossTelegraph(true);
        setBossNextMove("boss-heavy");
        announce("⚠️ ĐÒN NẶNG!", "#ef4444");
        addLog(`${boss.name} chuẩn bị đòn nặng!`, "info");
        ...
        if (evaded) {
          ...
          if (chosenMove.id === "boss-elemental") {
            ...
            setTimeout(() => {
              ...
              const noteStr = notes.length > 0 ? ` [${notes.join(", ")}]` : "";
              addLog(
                `${ELEMENT_EMOJI[boss.elementId] || "👹"} ${boss.name} dùng ${chosenMove.name}!${noteStr}`,
                "ability",
              );
              ...
              setBossTeam((prev) =>
                prev.map((c, i) =>
                  i === bossIdx
                    ? {
                        ...c,
                        [isPoison ? "poisonStacks" : "burnStacks"]:
                          (isPoison ? c.poisonStacks : c.burnStacks) +
                          (chosenMove.effect?.duration || 2),
                      }
                    : c,
                ),
              );
              ...
              prev.map((c, i) =>
                i === bossIdx
                  ? {
                      ...c,
                      shieldActive: true,
                      shieldValue: shieldVal,
                      shieldTurns: chosenMove.effect?.duration || 2,
                      dodgeCooldown: 3,
                    }
                  : c,
              ),
          ...
          // Heavy attack — telegraphed / called from helper
          ...
          // Normal tackle
            ...
            setTimeout(() => {
              ...
              const noteStr = notes.length > 0 ? ` [${notes.join(", ")}]` : "";
              addLog(
                `${ELEMENT_EMOJI[boss.elementId] || "👹"} ${boss.name} → ${dmg} dmg${noteStr}`,
                "damage",
              );
  ...
  const executeMove = useCallback(
    (move: BattleMove) => {
      ...
      if (!attacker?.isAlive || attacker.stunned > 0) {
        setPhase("idle");
        return;
      }
      ...
      setTimeout(() => {
            ...
            prev.map((c, i) =>
              i === activePlayerIdx
                ? {
                    ...
                    dodgeCooldown: 2,
                    moves: c.moves.map((m) =>
                      m.id === move.id ? { ...m, currentCooldown: 2 } : m,
                    ),
                  }
                : c,
            ),
        ...
        setTimeout(() => {
          ...
          if (effect?.type === "damage") {
            ...
            setPlayerTeam((prev) =>
              prev.map((c, i) =>
                i === activePlayerIdx ? { ...c, hp: Math.min(c.maxHp, c.hp + heal) } : c,
              ),
            );
            ...
            setBossTeam((prev) =>
              prev.map((c, i) =>
                i === bossIdx ? { ...c, poisonStacks: c.poisonStacks + (effect.duration || 2) } : c,
              ),
            );
            ...
            setBossTeam((prev) =>
              prev.map((c, i) =>
                i === bossIdx ? { ...c, burnStacks: c.burnStacks + (effect.duration || 2) } : c,
              ),
            );
            ...
            setBossTeam((prev) =>
              prev.map((c, i) => (i === bossIdx ? { ...c, stunned: effect.duration || 1 } : c)),
            );
  ...
  const startBattle = useCallback(() => {
    ...
    const bosses = level.bossIds.map((bossId) =>
      buildBattleCard(bossId, 1, level.bossAtkMult, level.bossHpMult),
    );
    setBossTeam(bosses);
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(180deg, #0a0a14 0%, #12121f 40%, #1a1030 70%, #0a0a14 100%)",
      }}
    >
      {/* ─── Top bar ──────────────────────────────────────────────────────── */}
      <div className="relative z-30 flex items-center justify-between px-2 sm:px-4 py-1.5 sm:py-2 border-b border-white/10 bg-black/60 backdrop-blur-xl">
        <Button
          ...
          variant="ghost"
          className="bg-white/8 text-white/80 border border-white/10 hover:bg-white/15 text-[10px] sm:text-xs px-2 sm:px-2.5 py-1 gap-1"
        >
          <ArrowLeft size={12} /> <span className="hidden sm:inline">Thoát</span>
        </Button>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <Badge tone="warning" className="text-[9px] sm:text-xs">
            ⚔️ Đấu Trường
          </Badge>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 rounded-full bg-black/50 px-1.5 sm:px-2 py-0.5 border border-white/10">
            ...
            <span className="text-[9px] sm:text-[10px] font-black text-emerald-400">
              {playerAlive.length}/{playerTeam.length}
            </span>
          </div>
        </div>
      </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div
                      className={`flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl ${selectedLevelId === lvl.id ? "bg-amber-500 text-white shadow-amber-500/30" : "bg-slate-800 text-slate-500"}`}
                    >
                      <Trophy size={18} />
                    </div>
        ...
        <ScreenShake active={isShaking}>
          <div className="relative flex flex-1 flex-col overflow-hidden">
            ...
            {playerAlive.length > 0 && (
              <DangerVignette
                playerHpPercent={
                  playerAlive[activePlayerIdx]
                    ? (playerAlive[activePlayerIdx].hp / playerAlive[activePlayerIdx].maxHp) * 100
                    : 100
                }
              />
            )}
            ...
            <AnimatePresence>
              {bossIntroBoss && (
                <BossIntroSlide
                  key="boss-intro"
                  bossName={bossIntroBoss.name}
                  elementId={bossIntroBoss.elementId}
                  bossCardId={bossIntroBoss.id}
                  bossRarityId={bossIntroBoss.rarityId}
                />
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {turnAnnounce && (
                <TurnAnnouncement key="announce" text={turnAnnounce} color={announceColor} />
              )}
            </AnimatePresence>
            ...
            <div className="relative z-10 border-b border-white/10 bg-gradient-to-b from-red-950/20 to-transparent px-1 sm:px-2 pt-1.5 sm:pt-2 pb-2 sm:pb-3">
              ...
              {/* Boss lineup */}
              <div className="flex items-end justify-center gap-1 sm:gap-2 mt-2 overflow-x-auto thin-scrollbar px-1">
                {bossTeam.map((b, i) => (
                  <BattleAvatar
                    key={b.id}
                    card={b}
              ...
              {/* Enrage warning banner */}
              {bossEnraged && (
                <motion.div
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-1 flex items-center justify-center gap-2 rounded-lg bg-red-900/60 border border-red-500/50 px-3 py-1"
                >
                  <motion.div
                  ...
                  <span className="text-[9px] sm:text-[10px] font-black text-red-300 uppercase tracking-wider">
                    Boss đã nổi giận! ATK +30%
                  </span>
                  <motion.div
                    animate={{ rotate: [5, -5, 5], scale: [1.2, 1, 1.2] }}
                    transition={{ duration: 0.4, repeat: Infinity }}
                    className="text-lg"
                  >
                    🔥
              ...
              {bossTelegraph && (
                <motion.div
                  ...
                  className="mt-1 flex items-center justify-center gap-2 rounded-lg bg-orange-900/60 border-2 border-orange-400/60 px-3 py-1.5"
                ...
                >
                  ...
                  <span className="text-[10px] sm:text-[11px] font-black text-orange-300 uppercase tracking-wider">
                    Cẩn thận! Đòn nặng sắp tới!
                  </span>
                  <motion.div
                    animate={{ y: [3, -3, 3], rotate: [10, -10, 10] }}
                    transition={{ duration: 0.5, repeat: Infinity }}
            ...
            <div className="relative z-10 flex-1 px-1 sm:px-2 py-1.5 sm:py-2 flex gap-1 sm:gap-2">
              ...
              <div className="flex flex-col items-center gap-1 sm:gap-2 flex-1">
                {/* Team lineup */}
                <div className="flex items-start justify-center gap-1 sm:gap-2 mb-0.5 sm:mb-1 overflow-x-auto thin-scrollbar px-1">
                  {playerTeam.map((p, i) => (
                  ...
                  <div className="w-full max-w-[150px] sm:max-w-[200px] space-y-0.5 sm:space-y-1">
                    <div className="flex items-center justify-between text-[7px] sm:text-[9px]">
                      ...
                      <span className="text-cyan-300 font-black">
                        {playerEnergy}/{activePlayer.maxEnergy}
                      </span>
                    </div>
            ...
            <AnimatePresence>
              {hitSparks && (
                <HitSparks key="sparks" target={hitSparks.target} elementId={hitSparks.elementId} />
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {effectBanner && (
                <EffectivenessBanner
                  key="banner"
                  info={effectBanner.info}
                  side={effectBanner.side}
                />
              )}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {stage === "victory" && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-md overflow-y-auto"
          >
            <VictoryConfetti />
            <motion.div
            ...
            >
              ...
              <motion.h2
                ...
                className="relative text-2xl sm:text-4xl font-black uppercase tracking-widest text-amber-400 drop-shadow-[0_0_20px_rgba(234,179,8,0.6)]"
              >
                Chiến Thắng!
              </motion.h2>
              <motion.p
              ...
              {maxComboRef.current >= 2 && (
                ...
                >
                  ...
                  <span className="text-xs text-amber-500">
                    (+{(1 + maxComboRef.current * 0.1).toFixed(1)}x DMG)
                  </span>
                </motion.div>
              )}
              ...
              {(() => {
                const key = `bmo:firstclear:${selectedLevelId}`;
                const wasFirst = localStorage.getItem(key) === "1";
                return !wasFirst ? (
                  <motion.div
                    initial={{ opacity: 0, scale: 0.7, y: -10 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    transition={{ delay: 0.5, type: "spring", stiffness: 300 }}
                    className="relative mt-3 flex items-center justify-center gap-2 rounded-xl border-2 border-cyan-400 bg-gradient-to-r from-cyan-900/40 to-blue-900/40 px-4 py-2"
                  ...
                  >
                    ...
                    <span className="ml-2 rounded-full bg-cyan-400 px-2 py-0.5 text-xs font-black text-cyan-900">
                      Huy hiệu
                    </span>
                  </motion.div>
                ) : null;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[10px] text-yellow-500">
                        Không nhận sát thương — thành tích đã được ghi nhận!
                      </p>
                    </div>
                  </div>
              ...
              >
                ...
                <div className="text-left">
                  <p className="text-[10px] text-amber-400/70 uppercase tracking-wider font-bold">
                    Phần thưởng
                  </p>
              ...
              <Button
                onClick={() => {
                  ...
                  onWin({ action: "card_battle", level: selectedLevelId });
                  onClose();
                }}
                size="lg"
