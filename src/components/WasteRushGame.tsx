...
import { Zap, Heart, X, Clock, Trophy, Star, ChevronRight } from "lucide-react";
...
export function WasteRushGame({ user, onComplete, onClose }: WasteRushGameProps) {
  ...
  const finishGame = useCallback(async () => {
    ...
    // Refs always contain the latest values, including when the timer callback
    // was created before the player answered later questions.
    const finalScore = scoreRef.current + (comboRef.current >= 3 ? comboRef.current * 5 : 0);
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--text-muted)]">{t("wasteRush.introDesc")}</p>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <ResultStat
            label="Đúng"
            value={correct}
            icon={<Star size={14} className="text-green-400" />}
          />
          <ResultStat
            label="Combo max"
            value={`x${mult}`}
            icon={<Trophy size={14} className="text-rose-400" />}
          />
        </div>
          ...
          <div className="rounded-2xl border border-amber-200/30 bg-amber-50/20 p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-amber-400">
                {result.rank <= 10
                  ? `Top #${result.rank} hôm nay!`
                  : `Top ${result.percentile}% hôm nay`}
              </span>
            </div>
          </div>
        )}
    ...
    <div className="flex flex-col gap-3 p-4">
      ...
      <div className="flex justify-between">
        ...
        <motion.div
          ...
          style={{
            background: `${getComboColor(combo)}30`,
            border: `1px solid ${getComboColor(combo)}50`,
          }}
        >
      ...
      <div className="grid grid-cols-2 gap-2">
        {(["plastic", "paper", "glass", "organic", "hazard", "metal"] as WasteCategory[]).map(
          (cat) => (
            <button
              key={cat}
            ...
            >
              <span className="text-lg">
                {BIN_COLORS[cat] === "#06b6d4"
                  ? "🟦"
                  : BIN_COLORS[cat] === "#f59e0b"
                    ? "🟨"
                    : BIN_COLORS[cat] === "#14b8a6"
                      ? "🟩"
                      : BIN_COLORS[cat] === "#64748b"
                        ? "⬜"
                        : BIN_COLORS[cat] === "#22c55e"
                          ? "🌿"
                          : "🟥"}
              </span>
      ...
      <div className="flex justify-between text-[10px] text-slate-600">
        <span>
          Item {currentIdx + 1}/{items.length}
        </span>
        <span>
