...
import type {
  ...
  TournamentBracket as TournamentBracketData,
} from "../types";
...
export function TournamentBracket({ onClose, currentUserNick }: Props) {
  ...
  const fetchBracket = useCallback(async () => {
    try {
      ...
      setBracket(
        data as { bracket: TournamentBracketData | null; participants: TournamentParticipant[] },
      );
      setStage("bracket");
    } catch {
      setStage("joined");
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Lỗi khi tham gia giải đấu");
    } finally {
      setJoining(false);
    }
  };
                ...
                >
                  ...
                  <div className="mb-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {REWARDS_DISPLAY.map((r) => (
                        <div
                          key={r.place}
                          className={`rounded-2xl border ${r.bg} ${r.border} p-3 text-center`}
                        >
                          <p
                            className={`mb-1 bg-gradient-to-r bg-clip-text text-xl font-black text-transparent ${r.color}`}
                          >
                            {r.exp} EXP
                          </p>
                          <p className="text-xs font-bold text-slate-500">{r.label}</p>
                          {r.badge && (
                            <p className="mt-1 text-[10px] font-black italic text-purple-600">
                              {r.badge}
                            </p>
                          )}
                        </div>
                  ...
                  <div className="mb-5 space-y-2 text-sm text-slate-600">
                    <div className="flex items-start gap-2">
                      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-purple-100 text-[9px] font-black text-purple-600">
                        1
                      </span>
                    ...
                    <div className="flex items-start gap-2">
                      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center rounded-full bg-purple-100 text-[9px] font-black text-purple-600">
                        3
                      </span>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-1">
                      {bracket?.participants?.slice(0, 10).map((p, i) => (
                        <div
                          key={p.userId}
                          className={`flex items-center justify-between rounded-xl px-3 py-2 ${p.userId === currentUserNick ? "bg-purple-50 border border-purple-200" : "bg-white border border-slate-100"}`}
                        >
                          <div className="flex items-center gap-2">
                            <span
                              className={`w-6 text-center text-xs font-black ${i === 0 ? "text-amber-500" : i === 1 ? "text-slate-400" : i === 2 ? "text-orange-400" : "text-slate-400"}`}
                            >
                              {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`}
                            </span>
                            <span
                              className={`text-sm font-bold ${p.userId === currentUserNick ? "text-purple-700" : "text-slate-700"}`}
                            >
                              {p.name}
                          ...
                          <span className="text-sm font-black text-slate-500">
                            {(p.weeklyScore || 0).toLocaleString()} EXP
                          </span>
                        </div>
                      ))}
                          ...
                          <div key={round.round}>
                            ...
                            <div className="space-y-2">
                              {round.matches.map((match) => (
                                <div
                                  key={match.id}
                                  className={`flex items-center justify-between rounded-xl border px-3 py-2 ${match.status === "live" ? "border-red-300 bg-red-50" : "border-slate-200 bg-white"}`}
                                >
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`w-1.5 rounded-full ${match.status === "live" ? "bg-red-500 animate-pulse" : "bg-slate-300"}`}
                                    />
                                    <div>
                                      <p
                                        className={`text-sm font-bold ${match.player1Id === currentUserNick ? "text-purple-700" : "text-slate-700"}`}
                                      >
                                        {match.player1Name}
                                      </p>
                                      {match.player2Id && (
                                        <p
                                          className={`text-sm font-bold ${match.player2Id === currentUserNick ? "text-purple-700" : "text-slate-700"}`}
                                        >
                                          {match.player2Name}
                                        </p>
                                      )}
                                  ...
                                  {match.status === "completed" && (
                                    <div className="flex items-center gap-1 text-xs font-bold text-slate-400">
                                      <Star
                                        size={12}
                                        className={
                                          match.winnerId === currentUserNick
                  ...
                  {stage === "joined" && !bracket?.bracket && (
                    <div className="mb-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
                      <p className="font-bold text-slate-500">
                        Cần ít nhất 2 người tham gia để xếp bracket
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        Bracket sẽ được tạo tự động khi đủ 2 người
                      </p>
                    </div>
                  )}
