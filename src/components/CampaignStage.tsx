...
export default function CampaignStage({
  ...
  const stageTitle =
    stage.type === "boss"
      ? t("campaign.stageBoss")
      : stage.type === "elite"
        ? t("campaign.stageElite")
        : stage.type === "miniboss"
          ? t("campaign.stageMiniboss")
          : t("campaign.stageNormal");
  const stageSubtitle =
    stage.type === "boss"
      ? t("campaign.stageBossHint")
      : stage.type === "elite"
        ? t("campaign.stageEliteHint")
        : stage.type === "miniboss"
          ? t("campaign.stageMinibossHint")
    ...
    <div className="fixed inset-0 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-white">{t(stage.nameKey) || stage.nameKey}</h1>
            </div>
            <p className="text-sm text-white/60">
              {t(region.nameKey) || region.nameKey} - {t("campaign.stage")}{" "}
              {region.stages.findIndex((s) => s.id === stageId) + 1}
            </p>
          </div>
      ...
      <div className="px-6 py-8 max-w-4xl mx-auto">
        ...
        <div className="mt-6 p-4 rounded-xl bg-amber-500/20 border border-amber-500/40 text-center">
          <p className="text-amber-300 text-sm">{t("campaign.placeholderHint")}</p>
          <a
            href="/world-map"
