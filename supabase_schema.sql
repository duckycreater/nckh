...
-- ─── Quiz Questions (admin-managed minigame) ─────────────────────────────────
CREATE TABLE IF NOT EXISTS quiz_questions (
  id SERIAL PRIMARY KEY,
  question_id INTEGER UNIQUE NOT NULL,
  content TEXT NOT NULL,
  options JSONB NOT NULL,                   -- [{key:"A",text:"..."}, {key:"B",...}, ...]
...
-- ─── Quiz Config (key-value runtime config) ─────────────────────────────────
...
CREATE TABLE IF NOT EXISTS admin_actions (
  ...
  action_type VARCHAR(50) NOT NULL,         -- 'quiz_create' | 'quiz_update' | 'quiz_delete' | 'sheets_sync' | 'reward_change' | 'user_ban' | ...
  target_type VARCHAR(50),                   -- 'quiz_question' | 'user' | 'reward' | ...
