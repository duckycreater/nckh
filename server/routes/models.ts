 ...
 *
 * Manifests are signed server-side for auditability; browsers validate the
 * schema and verify the pinned weight digest before loading bytes.
 */
...
export function modelsRouter(): Router {
  ...
  router.get(
    ...
    zodValidate({ params: ModelVersionParams }),
    async (req, res) => {
      try {
        ...
        signed.signature = signManifest(manifest);
