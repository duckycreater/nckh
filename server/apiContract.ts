...
/* ─── Domain enums ───────────────────────────────────────────────── */
export const WasteCategoryEnum = z.enum([
  "plastic",
  "paper",
  "glass",
  "metal",
...
export const Contracts = {
  ...
  "GET /api/federated/status": {
    response: z
      .object({
        ...
        latestVersion: z
          .object({ version: z.string(), trainedOn: z.number(), createdAt: z.number() })
          .nullable(),
        dp: z.object({ epsilon: z.number(), delta: z.number(), clipNorm: z.number() }),
      })
      .passthrough(),
    auth: false,
  },
  ...
  "GET /api/impact/summary": {
    query: z.object({
      cohort: z.string().default("global"),
      sinceDays: z.coerce.number().int().positive().default(30),
    }),
...
export function validateQuery<K extends ContractKey>(
  key: K,
...
export function validateResponse<K extends ContractKey>(key: K, raw: unknown): unknown {
  ...
  if (!parsed.success) {
    ...
    if (typeof console !== "undefined")
      console.warn(`[apiContract] ${key} response drift:`, parsed.error.message);
    return raw;
  }
  return parsed.data;
}
